                        ...
                        <div class="px-5 py-4 flex items-start gap-4 <?= $isRead ? '' : 'bg-indigo-50/50 dark:bg-indigo-900/10' ?> hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-2">
                                    <div>
                                        <p class="text-sm font-semibold text-gray-900 dark:text-white"><?= htmlspecialchars($notif->title ?? $notif['title'] ?? '') ?></p>
                            ...
                            <?php if (!$isRead): ?>
                                <form action="/notifications/<?= htmlspecialchars($notif->id ?? $notif['id'] ?? '') ?>/mark-read" method="POST" class="flex-shrink-0">
                                    <input type="hidden" name="_csrf_token" value="<?= htmlspecialchars($_SESSION['_csrf_token'] ?? '') ?>">
                                    <button type="submit" class="text-xs text-primary-700 dark:text-primary-400 hover:underline">Mark read</button>
                                </form>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
