...
$total = (float)($total ?? $subtotal - $discount);
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
    ...
    <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-6">Shopping Cart</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-4">
                <?php foreach ($cartItems as $item): ?>
                    ...
                    $ciStore = $item->store_name ?? $item['store_name'] ?? '';
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 cart-item">
                        <div class="flex gap-4">
                            <a href="/shop/<?= htmlspecialchars($ciSlug) ?>" class="w-20 h-20 sm:w-24 sm:h-24 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700 flex-shrink-0">
                                ...
                                <?php else: ?>
                                    <div class="w-full h-full flex items-center justify-center"><i class="fas fa-image text-gray-300 dark:text-gray-600 text-2xl"></i></div>
                                <?php endif; ?>
                            </a>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-2">
                                    <div>
                                        ...
                                        <?php if ($ciStore): ?>
                                            <p class="text-xs text-gray-400 mt-0.5"><i class="fas fa-store mr-1"></i><?= htmlspecialchars($ciStore) ?></p>
                                        <?php endif; ?>
                                    </div>
                                    <button type="button" class="remove-item-btn flex-shrink-0 p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition" data-item-id="<?= $ciId ?>" aria-label="Remove"><i class="fas fa-trash-alt text-sm"></i></button>
                                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                    <div class="flex flex-col sm:flex-row gap-3">
                        <div class="flex-1">
                            <input type="text" id="coupon-input" placeholder="Enter coupon code" class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 dark:text-gray-200 placeholder-gray-400" value="<?= htmlspecialchars($_GET['coupon'] ?? $coupon->code ?? '') ?>">
                        </div>
            ...
            <div>
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 sticky top-24">
                    ...
                    <a href="/checkout" class="mt-4 w-full py-3 bg-accent-600 hover:bg-accent-700 text-white font-semibold rounded-lg transition shadow-md hover:shadow-lg text-sm flex items-center justify-center gap-2">
