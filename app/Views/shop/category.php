...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-4 sm:mb-6 flex-wrap">
        ...
        <?php if (!empty($parentCategory)): ?>
            <i class="fas fa-chevron-right text-[10px]"></i>
            <a href="/shop/category/<?= htmlspecialchars($parentCategory->slug ?? $parentCategory['slug'] ?? '') ?>" class="hover:text-primary-700 dark:hover:text-primary-400 transition"><?= htmlspecialchars($parentCategory->name ?? $parentCategory['name'] ?? '') ?></a>
        <?php endif; ?>
        <i class="fas fa-chevron-right text-[10px]"></i>
    ...
    <div class="relative rounded-xl overflow-hidden mb-6 bg-gradient-to-r from-primary-800 to-primary-700">
        <?php if ($catBanner): ?>
        ...
        <div class="mb-6">
            <div class="flex items-center gap-2 overflow-x-auto scrollbar-hide pb-2">
                <?php foreach ($subcategories as $sub): ?>
                    ...
                    ?>
                    <a href="/shop/category/<?= htmlspecialchars($catSlug ?? $cat->slug ?? '') ?>?subcategory=<?= htmlspecialchars($subSlug) ?>"
    ...
    <?php if (!empty($products) && count($products) > 0): ?>
        <div class="flex items-center justify-between mb-4">
            ...
            <select onchange="window.location.href=this.value" class="px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 dark:text-gray-200 cursor-pointer">
                ...
                $baseUrl = '/shop/category/' . htmlspecialchars($catSlug ?? $cat->slug ?? '') . '?' . http_build_query(array_merge($_GET, ['sort' => '__VAL__']));
        ...
        <?php if (!empty($pagination) && $pagination['lastPage'] > 1): ?>
            <div class="flex items-center justify-center gap-1.5 mt-8">
                ...
                $urlTemplate = $pagination['urlTemplate'] ?? '/shop/category/' . htmlspecialchars($catSlug ?? $cat->slug ?? '') . '?page=__PAGE__';
                ?>
    ...
    <?php else: ?>
        <div class="text-center py-16 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
            ...
            <a href="/shop" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-700 hover:bg-primary-800 text-white text-sm font-medium rounded-lg transition shadow-sm"><i class="fas fa-store"></i> Browse All Products</a>
        </div>
    <?php endif; ?>
</div>
